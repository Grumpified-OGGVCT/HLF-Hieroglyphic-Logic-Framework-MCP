...
def _run_imports():
    ...
    # Test MCP resources
    try:
        print("  ✓ mcp_resources: HLFResourceProvider, Resource, ResourceTemplate")
        passed += 1
    ...
    # Test MCP tools
    try:
        print("  ✓ mcp_tools: HLFToolProvider, ToolDefinition")
        passed += 1
    except Exception as e:
        print(f"  ✗ mcp_tools: {e}")
        failed += 1
    ...
    # Test MCP prompts
    try:
        print("  ✓ mcp_prompts: HLFPromptProvider, PromptDefinition, PromptArgument")
        passed += 1
    except Exception as e:
    ...
    # Test MCP client
    try:
        print("  ✓ mcp_client: HLFMCPClient, GrammarInfo")
        passed += 1
    except Exception as e:
    ...
    # Test Forge agent
    try:
        print("  ✓ forge_agent: ForgeAgent, FrictionReport, GrammarProposal")
        passed += 1
    except Exception as e:
...
def _run_prompt_provider():
    ...
    try:
        ...
        init_prompt = provider.get_prompt(
        ...
        intent_prompt = provider.get_prompt(
            "hlf_express_intent", {"intent": "Read a file and write the result"}
        )
...
def _run_mcp_client():
    ...
    try:
        from hlf.mcp_client import GrammarInfo, HLFMCPClient
        ...
        print("  ✓ GrammarInfo dataclass works")
        passed += 1
