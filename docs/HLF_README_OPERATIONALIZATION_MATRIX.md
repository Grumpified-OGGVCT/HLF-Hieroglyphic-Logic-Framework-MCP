...
| Deterministic orchestration | `bridge work` | replay-determinism benchmark and orchestration trace proofs | execution cleanliness, swarm coordination quality | `README.md`, `HLF_QUALITY_TARGETS.md`, `docs/HLF_PILLAR_MAP.md` | Restore orchestration lifecycle and add replay-equivalence evidence |
...
| HLF knowledge substrate and governed memory | `bridge work` | unified evidence schema with provenance, freshness, confidence, trust-tier, supersession, and expiry tests | tool contract reliability, safety and governance quality | `README.md`, `SSOT_HLF_MCP.md`, `HLF_QUALITY_TARGETS.md`, `docs/HLF_PILLAR_MAP.md` | Finish memory-governance recovery spec and enforce runtime contracts |
| Instinct lifecycle | `bridge work` | orchestration recovery spec and deterministic lifecycle execution traces | swarm coordination quality | `README.md`, `SSOT_HLF_MCP.md`, `docs/HLF_PILLAR_MAP.md` | Connect current lifecycle fragments to plan execution and handoff lineage |
| Formal verification | `source-context evidence` | packaged verifier surface, proof serialization, and negative/positive verification regressions | execution cleanliness, safety and governance quality | `README.md`, `docs/HLF_REJECTED_EXTRACTION_AUDIT.md`, `docs/HLF_PILLAR_MAP.md` | Faithfully port verifier authority from upstream into packaged truth |
...
3. Extend `docs/HLF_MEMORY_GOVERNANCE_RECOVERY_SPEC.md` with the unified evidence contract needed by routing and promotion logic.
4. Add the missing replay-equivalence and round-trip proof artifacts before strengthening orchestration or five-surface language claims.
5. Keep stronger recursive-build and remote transport language in bridge-only phrasing until the repo-owned smoke path proves the intended MCP initialize flow end to end.
