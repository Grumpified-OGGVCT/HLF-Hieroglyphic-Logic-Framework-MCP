# HLF Full Architectural Manifesto

> **Note:** This is the deep architecture document preserving the original README's full content. For the quick-start README, see [../README.md](../README.md).

This document was extracted from the original 1643-line README.md on 2026-05-13 to create a lean, actionable front door while keeping all architectural depth accessible.
...
`install.bat` creates the local Python environment, prefers `uv.lock` with frozen `uv` sync when available, falls back to `requirements.txt`/pip compatibility, installs the full HLF MCP runtime/dev dependency set (`-e .[full,dev]`), installs the VS Code bridge/operator-shell Node dependencies, and verifies the live MCP surface plus operator CLI lane. `run.bat` defaults to MCP-safe `stdio` mode for local agents; it deliberately prints no stdout banner before the MCP server starts. Use `run.bat http 8123` for Streamable HTTP, `run.bat test` for pytest, `run.bat lint` for Ruff, and `run.bat extension-test` for the VS Code bridge tests.

Socket-restricted or offline Windows machines can use the no-fetch path:

```bat
install.bat --offline
install.bat offline-check
```

Offline mode does not upgrade pip tooling and does not contact package indexes. It uses a repo-local `.venv`, local `uv` cache with `uv.lock`/`--frozen --offline`, or a repo-local `wheelhouse` for pip `--no-index` installs. If the Python or VS Code bridge dependencies are not already available locally, it fails with explicit actions: seed the uv/pip/npm caches from an online machine, copy a prepared `.venv`/`extensions\hlf-vscode\node_modules`, or place all wheels required by `requirements.txt` under `wheelhouse`.

Fresh-client MCP wiring should avoid PATH-dependent `python` or global `uv` assumptions. The repo `.mcp.json` and VS Code workspace MCP config launch the local stdio server through `run.bat`; private machine-specific client configs may use absolute paths to `run.bat`, `.venv\Scripts\python.exe -m hlf_mcp.server`, or `uv run hlf-mcp` only when those paths/tools are guaranteed on that machine.
...
The Docker lane builds `grumprolled/hlf-mcp-mouthpiece:local` and runs the named container `grumprolled-hlf-mcp` on Streamable HTTP (`HLF_TRANSPORT=streamable-http`, default `HLF_PORT=8123`). It is a complementary global/containerized MCP mouthpiece for users who want a system-wide HLF endpoint; it does not replace the local `stdio`/CLI lane for native agents, onboarding, or recursive-build proof. Docker installs dependencies from `uv.lock` with `uv sync --frozen`; if the lock and project metadata diverge, the image build is expected to fail instead of resolving a fresh dependency set. Compose persists `/app/db` and `/app/data` through named volumes.
...
### Option A — Docker mouthpiece (complementary containerized lane)

```bash
# Build the local mouthpiece image from the frozen uv lock.
docker build -t grumprolled/hlf-mcp-mouthpiece:local .

# Default container transport is Streamable HTTP on port 8123.
docker run --rm --name grumprolled-hlf-mcp -p 8123:8123 grumprolled/hlf-mcp-mouthpiece:local

# The image still exposes the same packaged server if a caller explicitly chooses SSE or stdio.
docker run --rm --name grumprolled-hlf-mcp -e HLF_TRANSPORT=sse -e HLF_PORT=<explicit-port> -p <explicit-port>:<explicit-port> grumprolled/hlf-mcp-mouthpiece:local
docker run --rm -i -e HLF_TRANSPORT=stdio grumprolled/hlf-mcp-mouthpiece:local
```

Docker is a complementary global/containerized MCP mouthpiece lane, not a replacement for local `stdio` or the packaged CLI. The local `stdio`/CLI path remains the default native-agent and recursive-build proof lane; Docker is for operators who want a named, system-wide container endpoint. The Dockerfile copies `uv.lock` and uses `uv sync --frozen` so dependency drift blocks the image build instead of silently resolving new package versions.

For HTTP transports outside the compose default, choose and set the port explicitly. The packaged server no longer treats `8000` as an implied default for `sse` or `streamable-http`.
...
```bash
docker compose up -d
# MCP Streamable HTTP server → http://localhost:8123/mcp
# Health check                 → http://localhost:8123/health

HLF_PORT=<explicit-port> docker compose up -d
# MCP Streamable HTTP server → http://localhost:$HLF_PORT/mcp
...
The Docker lane is the complementary global/containerized MCP mouthpiece. It keeps the local `stdio`/CLI workflow intact for native agents and recursive-build proof while giving operators a named container endpoint.

### Lock-aware image build

The checked-in `Dockerfile` builds `grumprolled/hlf-mcp-mouthpiece:local` and copies both `pyproject.toml` and `uv.lock` before installing dependencies. Dependency installation uses `uv sync --frozen --no-dev`, so Docker builds are lock-aware and fail when the lock no longer matches project metadata. Optional runtime extras are selected with `HLF_INSTALL_EXTRAS` and must already be present in `uv.lock`; the build does not fall back to an unlocked `pip install`.

```bash
docker build -t grumprolled/hlf-mcp-mouthpiece:local .
docker build --build-arg HLF_INSTALL_EXTRAS=hot -t grumprolled/hlf-mcp-mouthpiece:local .
```
...
Compose preserves the required names unless intentionally changed by an operator:

- image: `grumprolled/hlf-mcp-mouthpiece:local`
- container: `grumprolled-hlf-mcp`
- default transport: `HLF_TRANSPORT=streamable-http`
- default port: `8123`
- health endpoint: `GET /health` inside the container
- persistent volumes: `hlf-mcp-db:/app/db` and `hlf-mcp-data:/app/data`

Hot/full profiles add Valkey without replacing the core mouthpiece service.

### Environment Reference

| Variable | Default | Description |
| --- | --- | --- |
| `HLF_TRANSPORT` | `streamable-http` in Docker, `stdio` in local scripts | Transport type: `stdio` / `sse` / `streamable-http` |
| `HLF_HOST` | `0.0.0.0` in Docker | Bind address for HTTP transports |
| `HLF_PORT` | `8123` in Docker | Bind port for HTTP transports |
| `HLF_INSTALL_EXTRAS` | empty | Optional locked extras for image build, for example `hot` or `full` |
