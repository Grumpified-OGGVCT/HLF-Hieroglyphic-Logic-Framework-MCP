...
class TestCheckEvidenceFreshnessMandatory:
    ...
    def test_fresh_passes(self, purpose: str) -> None:
        ...
        v = check_evidence_freshness(evidence_row=evidence, purpose=purpose)
    ...
    def test_superseded_rejected_for_execution_admission(self) -> None:
        ...
        v = check_evidence_freshness(evidence_row=evidence, purpose="execution_admission")
        assert v.admissible is False
...
class TestCheckEvidenceFreshnessEdgeCases:
    ...
    def test_missing_purpose_defaults_to_unrestricted(self) -> None:
        ...
        v = check_evidence_freshness(evidence_row=evidence)
        assert v.admissible is True
...
class TestResolveSupersessionChain:
    ...
    def test_single_link_chain(self) -> None:
        rows = [
            ...
            {"sha256": "b", "superseded_by_sha256": "", "revoked": False, "tombstoned": False},
        ]
        result = resolve_supersession_chain(sha256="a", rows=rows)
        assert result["head"] == "a"
        assert result["terminal"] == "b"
    ...
    def test_multi_link_chain(self) -> None:
        rows = [
            ...
            {"sha256": "c", "superseded_by_sha256": "d", "revoked": False, "tombstoned": False},
            {"sha256": "d", "superseded_by_sha256": "", "revoked": False, "tombstoned": False},
        ]
        result = resolve_supersession_chain(sha256="a", rows=rows)
        assert result["head"] == "a"
        assert result["terminal"] == "d"
        assert result["length"] == 4

    def test_not_found_in_rows(self) -> None:
        rows: list[dict[str, Any]] = []
        result = resolve_supersession_chain(sha256="missing", rows=rows)
        assert result["head"] == "missing"
        # chain empty, sha256 truthy → terminal == sha256
    ...
    def test_max_depth_enforced(self) -> None:
        ...
        result = resolve_supersession_chain(sha256="h0", rows=rows, max_depth=50)
    ...
    def test_mid_chain_lookup(self) -> None:
        ...
        rows = [
            {"sha256": "a", "superseded_by_sha256": "b", "revoked": False, "tombstoned": False},
            {"sha256": "b", "superseded_by_sha256": "c", "revoked": False, "tombstoned": False},
            {"sha256": "c", "superseded_by_sha256": "", "revoked": False, "tombstoned": False},
        ]
        result = resolve_supersession_chain(sha256="b", rows=rows)
...
class TestMissionEvidenceFreshness:
    ...
    def test_superseded_in_trace(self) -> None:
        ...
        result = check(mission)
        assert result["status"] == "ok"
        # superseded evidence is rejected for execution_admission purpose
        assert result["verdict"] == "stale"
    ...
    def test_verification_report_evidence(self) -> None:
        ...
        result = check(mission)
        assert result["status"] == "ok"
        # Only execution_trace is checked; verification_report is not scanned
        assert result["verdict"] == "fresh"
    ...
    def test_orchestration_contract_evidence(self) -> None:
        ...
        result = check(mission)
        assert result["status"] == "ok"
        # Only execution_trace is checked; orchestration_contract is not scanned
        assert result["verdict"] == "fresh"
    ...
    def test_non_dict_evidence_skipped(self) -> None:
        ...
        result = check(mission)
        assert result["status"] == "ok"
        # String evidence should be caught by from_dict fallback
        assert result["verdict"] in ("fresh", "stale")
