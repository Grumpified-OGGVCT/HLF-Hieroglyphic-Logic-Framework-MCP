...
| `hlf_source/agents/core/formal_verifier.py` | `missing`, `maybe` | strong but misaligned | Directly encodes satisfiability, invariant, gas-budget, reachability, and spec-gate verification concepts that are constitutive to trustworthy HLF execution. A thin packaged verifier boundary now exists, but it does not yet carry the upstream proof semantics. | runtime/proof | `faithful_port` | Top-priority recovery surface; current packaged proof objects are real but underpowered |
| `hlf_source/agents/core/plan_executor.py` | not extracted, treated as upstream workflow | strong but not yet packaged | Converts plans into DAG execution order with agent assignment and verification steps. This is a core multi-agent coordination surface, not generic automation. A packaged lifecycle and orchestration base now exists, but the execute path remains incomplete. | runtime/orchestration | `faithful_port` | Must be restored with clear packaged ownership boundaries and attached to the existing instinct surface |
| `hlf_source/agents/core/crew_orchestrator.py` | not extracted, treated as upstream workflow | strong but not yet packaged | Preserves multi-persona round-robin synthesis, SDD lifecycle ordering, and consolidation protocol. Packaged lifecycle state exists already, but coordinated execution and synthesis semantics are still missing. | runtime/orchestration | `faithful_port` | Strong evidence that lifecycle or orchestration is part of HLF’s intended shape rather than optional OS glue |
