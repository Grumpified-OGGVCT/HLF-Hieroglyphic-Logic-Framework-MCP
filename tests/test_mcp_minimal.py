...
class TestMCPTools:
    ...
    def test_friction_log(self, tmp_path):
        ...
        repo_root = Path(__file__).parent.parent
        resource_provider = HLFResourceProvider(repo_root)
        tool_provider = HLFToolProvider(resource_provider, friction_drop=tmp_path / "friction")
...
class TestMCPPrompts:
    ...
    def test_prompt_definition(self):
        ...
        from hlf.mcp_prompts import PromptArgument, PromptDefinition
...
class TestFrictionReport:
    ...
    def test_grammar_proposal_structure(self):
        ...
        assert proposal.additive_only
        assert not proposal.breaking
