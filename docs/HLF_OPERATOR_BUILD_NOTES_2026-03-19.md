...
| HLF knowledge substrate and memory governance | `hlf_mcp/rag/memory.py`, `hlf_mcp/hlf/memory_node.py`, `hlf_mcp/server_memory.py` | Infinite RAG and governed retrieval are real, but freshness, supersession, trust-tier, expiry, and one runtime schema still need to be locked down coherently |
