...
- **REQ-004**: Use `SSOT_HLF_MCP.md` as the present-tense build truth boundary only, not as the authority that shrinks doctrine or bridge scope.
...
- **REQ-011**: The repo must define a staged recursive build path where packaged HLF can assist the recovery of fuller HLF without overstating current transport readiness.
...
- **ARC-004**: Documentation must not be simplified to fit current packaged limitations except in explicit current-truth surfaces such as `SSOT_HLF_MCP.md`.
...
| TASK-006 | Update `HLF_ACTIONABLE_PLAN.md` to point readers at `plan/architecture-hlf-reconstruction-2.md` as the master reconstruction sequencing artifact. | ✅ | 2026-03-19 |
| TASK-007 | Add a brief reference to `plan/architecture-hlf-reconstruction-2.md` in `docs/HLF_STITCHED_SYSTEM_VIEW.md` so the stitched overview points to an execution plan, not just concept docs. | ✅ | 2026-03-19 |
| TASK-008 | Add a brief reference to `plan/architecture-hlf-reconstruction-2.md` in `README.md` under the bridge reading path so new readers can find the actual recovery sequence. | ✅ | 2026-03-19 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-009 | Create `docs/HLF_REJECTED_EXTRACTION_AUDIT.md` covering every source surface currently treated as `missing`, `superseded`, `optional`, `process-only`, or `OS-bound` where the surface may still be constitutive to HLF. | ✅ | 2026-03-19 |
| TASK-010 | For each surface in `docs/HLF_REJECTED_EXTRACTION_AUDIT.md`, record: upstream path, current ledger classification, corrected classification, why it matters, whether it is runtime, doctrine, operator, or source-only context, and proposed disposition. | ✅ | 2026-03-19 |
| TASK-011 | Seed the audit with the top ranked files already identified in `HLF_SUPPORTIVE_SOURCE_CONTEXT_MAP.md`: `hlf_source/AGENTS.md`, `hlf_source/agents/gateway/bus.py`, `hlf_source/agents/gateway/router.py`, `hlf_source/agents/core/formal_verifier.py`, `hlf_source/agents/core/plan_executor.py`, `hlf_source/agents/core/crew_orchestrator.py`, `hlf_source/config/personas/steward.md`, `hlf_source/governance/ALIGN_LEDGER.yaml`, `hlf_source/docs/UNIFIED_ECOSYSTEM_ROADMAP.md`, and `hlf_source/scripts/run_hlf_gallery.py`. | ✅ | 2026-03-19 |
| TASK-012 | Add a deterministic disposition field for every audited file with only these values: `restore`, `faithful_port`, `bridge_contract`, `source_only_for_now`, `not_hlf_core`. | ✅ | 2026-03-19 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-013 | Create `docs/HLF_PILLAR_MAP.md` with one section per pillar: semantic core, governance spine, effect algebra, runtime/bytecode, routing fabric, orchestration lifecycle, formal verification, HLF knowledge substrate / memory / audit, persona doctrine, ecosystem integration, operator gallery/trust surfaces. | ✅ | 2026-03-19 |
| TASK-014 | For each pillar, map the local ownership boundary across `hlf_mcp/`, `hlf/`, `governance/`, `docs/`, and `hlf_source/`. | ✅ | 2026-03-19 |
| TASK-015 | For each pillar, record the current state using only the approved damage classes and name the exact files supporting that decision. | ✅ | 2026-03-19 |
| TASK-016 | For each pillar, record the preferred next action using only the approved dispositions: `restore`, `faithful_port`, `bridge_contract`, or `source_only_for_now`. | ✅ | 2026-03-19 |

### Implementation Phase 3A

- **GOAL-003A**: Add an operator-facing handoff surface that states what is present, what is bridge work, what is source-only, and why.

| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-016A | Create `docs/HLF_OPERATOR_BUILD_NOTES_2026-03-19.md` to give future sessions and operators a concise bridge-lane build note that separates packaged truth, bridge work, and source-only constitutive surfaces. | ✅ | 2026-03-19 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-017 | Create `docs/HLF_README_OPERATIONALIZATION_MATRIX.md` mapping major public claims to one of: `implemented now`, `bridge work`, `vision only`, `source-context evidence`. | ✅ | 2026-03-19 |
| TASK-018 | Add rows for these specific claim families: governed meaning layer, deterministic orchestration, five-surface language, cryptographic governance, gas metering, memory substrate, Instinct lifecycle, formal verification, routing, code generation, human-readable audit, ecosystem integration. | ✅ | 2026-03-19 |
| TASK-019 | For every row marked `bridge work`, define the proving artifact required: regression tests, benchmark, governance control matrix, trace artifact, generated inventory, code restoration spec, or runtime contract. | ✅ | 2026-03-19 |
| TASK-020 | Link each matrix row back to `README.md`, `SSOT_HLF_MCP.md`, `HLF_QUALITY_TARGETS.md`, and the relevant pillar section in `docs/HLF_PILLAR_MAP.md`. | ✅ | 2026-03-19 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-021 | Create `docs/HLF_ROUTING_RECOVERY_SPEC.md` covering `hlf_source/agents/gateway/bus.py`, `hlf_source/agents/gateway/router.py`, and `hlf_source/agents/gateway/sentinel_gate.py`: what should be restored, what should become a bridge contract, and what remains source-only. | ✅ | 2026-03-19 |
| TASK-022 | Create `docs/HLF_FORMAL_VERIFICATION_RECOVERY_SPEC.md` covering `hlf_source/agents/core/formal_verifier.py` and any related current local verification surfaces. | ✅ | 2026-03-19 |
| TASK-023 | Create `docs/HLF_ORCHESTRATION_RECOVERY_SPEC.md` covering `hlf_source/agents/core/plan_executor.py`, `hlf_source/agents/core/crew_orchestrator.py`, and `hlf_source/agents/core/task_classifier.py`. | ✅ | 2026-03-19 |
| TASK-024 | Create `docs/HLF_MEMORY_GOVERNANCE_RECOVERY_SPEC.md` covering the HLF knowledge substrate and governed memory contracts, including provenance schema, freshness, trust tiers, context pruning, and chain verification. | ✅ | 2026-03-19 |
| TASK-025 | Create `docs/HLF_PERSONA_AND_OPERATOR_RECOVERY_SPEC.md` covering `hlf_source/config/personas/*.md`, `hlf_source/AGENTS.md`, and how persona doctrine should influence runtime, docs, or operator guidance without becoming uncontrolled code authority. | ✅ | 2026-03-19 |
| TASK-026 | Create `docs/HLF_GALLERY_AND_OPERATOR_SURFACES_SPEC.md` covering `hlf_source/scripts/run_hlf_gallery.py`, explainer surfaces, and operator-legibility assets. | ✅ | 2026-03-19 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-027 | Create `docs/HLF_RECOVERY_BATCH_1.md` naming the first implementation batch. This batch must include exact upstream files, exact target files, the ownership module, required tests, required docs updates, and acceptance gates. | ✅ | 2026-03-19 |
| TASK-028 | Create `docs/HLF_RECOVERY_BATCH_2.md` naming the second implementation batch using the same structure. | ✅ | 2026-03-19 |
| TASK-029 | Rank batch priority using architectural importance first: routing fabric, formal verification, orchestration lifecycle, HLF knowledge substrate and memory governance, persona doctrine integration, gallery/operator surfaces. | ✅ | 2026-03-19 |
| TASK-030 | Add an explicit reason for every pillar not selected for Batch 1 so the repo does not silently forget unselected work. | ✅ | 2026-03-19 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-031 | Create `plan/feature-routing-fabric-1.md` if Batch 1 starts with routing. Include file-level steps, target tests, dependency order, and rollback points. | ✅ | 2026-03-19 |
| TASK-032 | Create `plan/feature-formal-verifier-1.md` if Batch 1 includes formal verification. Include target module ownership and required governance/CLI/MCP exposure decisions. | ✅ | 2026-03-19 |
| TASK-033 | Create `plan/feature-orchestration-lifecycle-1.md` if Batch 1 includes orchestration surfaces. Include contract boundaries so runtime and operator-layer code do not get mixed blindly. | ✅ | 2026-03-19 |
| TASK-034 | Add a deterministic decision rule in each feature plan stating what belongs in packaged runtime, what belongs in docs/operator surfaces, and what stays source-only for now. | ✅ | 2026-03-19 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-035 | Create `docs/HLF_GOVERNANCE_CONTROL_MATRIX.md` mapping manifest assets, align rules, host-function contracts, capsules, and any restored routing/verifier surfaces to operational controls. | ✅ | 2026-03-19 |
| TASK-036 | Create `docs/HLF_RECOVERY_ACCEPTANCE_GATES.md` listing the exact rules a restoration must meet before it can be marked complete. | ✅ | 2026-03-19 |
| TASK-037 | Define the minimum regression suite additions required for each restoration class: runtime, routing, verifier, orchestration, memory, persona/operator. | ✅ | 2026-03-19 |
| TASK-038 | Define the minimum generated artifact refresh required for each restoration class: docs generation, fixture coverage, manifest checks, resource/tool counts, gallery output, trace verification. | ✅ | 2026-03-19 |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-039 | Update `HLF_MCP_TODO.md` so recovery tasks are grouped by pillar and batch, not just by historical topic accumulation. | ✅ | 2026-03-19 |
| TASK-040 | Update `TODO.md` so the high-level backlog points to the master plan, batch docs, and recovery specs instead of duplicating them loosely. | ✅ | 2026-03-19 |
| TASK-041 | Update `HLF_ACTIONABLE_PLAN.md` to retain its broader implementation value while explicitly deferring reconstruction sequencing authority to `plan/architecture-hlf-reconstruction-2.md`. | ✅ | 2026-03-19 |
| TASK-042 | Verify that `AGENTS.md`, `README.md`, and `docs/HLF_STITCHED_SYSTEM_VIEW.md` all point to the same current planning entry points. | ✅ | 2026-03-19 |

### Implementation Phase 10

- **GOAL-010**: Stage the first honest recursive build milestone without overstating HTTP self-hosting readiness.
