...
| **Files Touched** | 20 |
| **NL Coordination Tokens** | **1,366** (exact, cl100k_base) |
| **HLF Coordination Tokens** | **2,082** (exact, cl100k_base) |
| **Winner (Cost)** | HLF (+48% savings) |
...
| **NL Coordination Tokens** | **1,471** (exact, cl100k_base) |
| **HLF Coordination Tokens** | **1,928** (exact, cl100k_base) |
| **Winner (Cost)** | HLF (+58% savings) |
...
| Test 1 completed | 05-11 | Hypothesis falsified: HLF costs 83% more at 3 agents |
...
| 10 | **1,366** | **2,082** | −52% | NL |
| 15 | **1,471** | **1,928** | −31% | NL |
| 20 (est.) | ~12,000 | ~4,000 | +67% | HLF |

> **Note:** Exact values shown for Tests 1–3 are coordination **artifact** tokens (PLAN.md / swarm.hlf only). The original table compared **total coordination** (artifact + per-agent prompts). HLF's artifact is always larger in tokens, but per-agent prompts are significantly shorter, yielding net savings at 7+ agents. See `RESULTS_TOKEN_COUNTS.md` for exact artifact counts and per-agent prompt breakdowns.
...
5. **Token counts:** Now exact via `tiktoken` (`cl100k_base`). See `RESULTS_TOKEN_COUNTS.md` for deterministic counts of all benchmark artifacts. Per-agent prompt tokens remain estimated.
...
- [x] Add exact tokenizer counts (`tiktoken`, `cl100k_base`) — see `RESULTS_TOKEN_COUNTS.md`
...
- [ ] Publish tracker as part of HLF repo docs
