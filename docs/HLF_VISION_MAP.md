...
| Memory as governed substrate | Memory should carry provenance, freshness, confidence, and trust, not just storage | `hlf_mcp/rag/memory.py`, `hlf_mcp/hlf/memory_node.py`, `docs/INSTINCT_REFERENCE.md`, `HLF_MCP_TODO.md` | `hlf_source/agents/core/memory_scribe.py`, `hlf_source/agents/core/context_pruner.py`, `hlf_source/scripts/verify_chain.py` | This pillar exists in partial form and needs stronger contracts and audit shape |
